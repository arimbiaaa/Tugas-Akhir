...
void setup() {
  ...
  Firebase.begin("https://tugas-akhir-cffaa-default-rtdb.firebaseio.com/", "dDx4iIpPkRuZEtYSi2kSYVbwxQcSdKgpNVF1mlPL");
  ...
  if (cam.jpg2rgb(fb, &image_rgb)){
    ...
    if (!digitalRead(BUTTON_QR)){
      ...
      if (res.status){
        ...
        Firebase.setString(dataresi,"NomorResi/ResiAlat",res.payload);
        delay(1000);
